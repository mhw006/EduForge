...
export async function sendChatMessage({ message, sessionId, userProfile }) {
  const response = await apiFetch('/chat', {
    method: 'POST',
    demoUser: 'student',
    body: { message, sessionId, userProfile },
  })
  return handleResponse(response)
}

export async function updateChatProfile({ sessionId, userProfile }) {
  const response = await apiFetch('/chat/profile', {
    method: 'POST',
    demoUser: 'student',
    body: { sessionId, userProfile },
  })
  return handleResponse(response)
}

export async function getChatSession(sessionId) {
  const response = await apiFetch(`/chat/session/${sessionId}`, { demoUser: 'student' })
  return handleResponse(response)
}

export async function runChatDiagnostic({ sessionId, subject, grade, readingPreference, connectivity }) {
  const response = await apiFetch('/chat/diagnostic', {
    method: 'POST',
    demoUser: 'student',
    body: { sessionId, subject, grade, readingPreference, connectivity },
  })
  return handleResponse(response)
}
...
export async function getLoopStatus(classId) {
  try {
    const response = await apiFetch(`/analytics/loop-status/${classId}`, { demoUser: 'teacher' })
    return handleResponse(response)
  } catch {
    // Return mock data if no class exists yet (demo stability)
    return {
...
export async function getClassRecommendations(classId) {
  try {
    const response = await apiFetch(`/analytics/recommendations/${classId}`, { demoUser: 'teacher' })
    return handleResponse(response)
  } catch {
    return {
      recommendations: [
      ...
      detectedIssues: ['Students struggled with: Equivalent Fractions, Ratios', 'Most rewritten section: MAIN_CONTENT'],
      urgentAction: null,
      _mock: true,
    }
  }
...
export async function getClassAnalytics(classId) {
  try {
    const response = await apiFetch(`/analytics/${classId}`, { demoUser: 'teacher' })
    return handleResponse(response)
  } catch {
    return {
      totalStudents: 12,
